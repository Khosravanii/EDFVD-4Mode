            ...
            while time<lcm:
                for i in range(n):
                    ...
                    time==tasks[i][6])) or (last_criticality!=criticality and timeLeft[i]==tasks[i][1]):
                        if criticality==1 or criticality==3:
                            timeLeft[i]=tasks[i][2]
                        elif criticality==0 or criticality==2:
                ...
                for j in range(len(instances)):
                    ...
                    if timeLeft[instances[j][0][0]-1]>0:
                        ...
                        for k in range(j+1,len(instances)):    
                            if j<len(instances)-1 and \
                                ...
                                or (instances[j][0][5]==1 and instances[k][0][5]==1)))) and (((criticality==0 or criticality==2) and \
                                instances[k][1]+tasks_LO[instances[k][0][0]-1][3]<instances[j][1]+tasks_LO[instances[j][0][0]-1][3]) or \
                                ((criticality==1 or criticality==3 ) and instances[k][1]+instances[k][0][3]<instances[j][1]+instances[j][0][3])):
                                
                                instances[j],instances[k]=instances[k],instances[j] 
                                
                        if criticality==1 and instances[j][0][5]==0:
                            criticality=0
                        if criticality==0 or criticality==2 or ((criticality==1 or criticality==3) and instances[j][0][5]>0):
                    ...
                    #Fault Mode
                    if anyrun==1 and instances[j][0][5]>0 and timeLeft[instances[j][0][0]-1]>=0 and criticality ==0:
                        #error=int(input("Task ("+str(instances[j][0][0])+") Time ("+str(time)+") fault %d/0:"%(fault)))
                        error =r.randint(1, 2)
                        if fault==error:
                            print('Fault occured for Task %s at %s'%(instances[j][0][0],time+1))
                            timeLeft[instances[j][0][0]-1]=(instances[j][0][1])
                            criticality=2
                            is_pop = False
                            is_break = True
                    #High Mode
                    if anyrun==1 and instances[j][0][5]>0 and timeLeft[instances[j][0][0]-1]>=0 and (criticality ==1 or criticality ==2):
                        if criticality ==1 and timeLeft[instances[j][0][0]-1]>=0:
                            #error=int(input("Task ("+str(instances[j][0][0])+") Time ("+str(time)+") fault %d/0:"%(fault)))
                            error =r.randint(1, 2)
                            if fault==error:
                                print('Fault occured for Task %s at %s'%(instances[j][0][0],time+1))
                                timeLeft[instances[j][0][0]-1]=(instances[j][0][2])
                                temp=1
                                for k in range(j+1,len(instances)):
                                    if instances[k][0][0]== instances[j][0][0]:
                                        temp=k
                                ...
                                is_pop = False
                                is_break = True
                        elif criticality ==2 and timeLeft[instances[j][0][0]-1]==0:
                            #signal=int(input("Task ("+str(instances[j][0][0])+") Time ("+str(time)+") signal %d/0:"%(sig)))
                            signal =r.randint(1, 2)
                            if sig==signal:
                                print('No Completion Signal for Task %s at %s'%(instances[j][0][0],time+1))
